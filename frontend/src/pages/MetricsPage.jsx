        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card>
            ...
            <div className="relative h-72">
              {metricsError ? (
                <div className="flex h-full items-center justify-center text-base text-text-muted">
                  unavailable
                </div>
              ) : (
                <>
          ...
          <Card>
            ...
            <div className="max-h-80 overflow-y-auto space-y-2 pr-1">
              {logsError ? (
                <div className="flex items-center justify-center py-10 text-base text-text-muted">
                  unavailable
                </div>
              ) : logs.length === 0 ? (
                <div className="flex items-center justify-center py-10 text-base text-text-muted">
                  ...
                  <div key={log.id || log._id || idx} className="flex items-start gap-3 rounded-md border border-border bg-surface-100/40 px-3 py-2">
                    <div className="min-w-0 flex-1 space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-text-muted font-mono">{timeAgo(log.timestamp || log.createdAt)}</span>
                        <LogEventBadge event={log.event} />
                      </div>
                      <p className="text-base text-text">{log.message || log.text || JSON.stringify(log)}</p>
                    </div>
                  </div>
                ))
              )}
